function [dataset,num_sample]=custom_load_dataset(d,ratio,flag,Names,idxtop5)
...
if(size(token,1)~=0)
    ...
    for i=1:size(I,2) %%read the sample files in increasing order of the digits
        ...
        if(i==1)
            ...
            all_data(:,:,i)=data; %used to make sure previous data isn't being overwritten
        end
        if(i>1)
...
eeg_data=all_data;
eeg_data=all_data(:,idxtop5,(1:size(files,1)));
num_sample=size(all_data,3);
%files = dir(fullfile(d, 'tar*.csv'));%%choose all the files starting with tar
files = dir(fullfile(d, 'tar_test2.csv'));%%choose all the files starting with tar
...
dataset.target_value_for_training=dataset.target_value;
dataset.target_value_for_validation=dataset.target_value;
