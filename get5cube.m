function neucube=get5cube(neucube,idxtop5,folder_name)

%% proceed to update locked cube with input features
...
neucube.input_mapping(1:2) = {neucube.input_mapping{1}(idxtop5,:), neucube.input_mapping{2}(idxtop5,:)}; %change input mapping to top 5 features

neuinput=neucube.input_mapping{1}; %this is the input variable for features and their coordinates
neuron_location=cat(1,neucube.neuron_location(1:end-5,:),neuinput); %these are 1471+5 talairach neuron coordinates 
neucube.neuron_location=neuron_location
neucube.number_of_neucube_neural=size(neuron_location,1)
number_of_neucube_neural=neucube.number_of_neucube_neural %1471reservoir+5input neurons count
...
cubename5=strcat(folder_name,'get5cube.mat')
save(cubename5,'neucube')
